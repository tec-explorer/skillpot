...
只需一条命令即可在浏览器中开启现代化的 Skill 供应链与跨 Agent 治理控制台：

```bash
skillpot gui                # 默认启动并自动打开浏览器（http://127.0.0.1:3721）
skillpot gui --port 8080    # 自定义端口
skillpot gui --host 0.0.0.0 # 局域网共享模式（强制 Token 认证）
```
...
| 功能模块 | 界面与交互能力 | 业务价值 |
|---|---|---|
| **开关矩阵 (Matrix)** | 行是 Skill，列是 Agent 与通用广播；视口防溢出 Tooltip、行级/列级一键全开全停、目标 Agent 筛选、卡片吸附底栏分页 | 一屏总览全局 Skill 开放拓扑，毫秒级就地开关 |
| **适配顾问 (Advisor)** | 启发式 Token 快速估算、运行时依赖静态探测、4 级量化适配诊断（🟢推荐 / ⚪按需 / 🟡留意 / 🔴不兼容） | 预防上下文溢出与外部脚本依赖缺失 |
| **详情弹窗 (DetailModal)** | 展示描述、文件树、`SKILL.md` 正文代码、安全评级，各 Agent 适配卡片**就地集成 Toggle 开关**，支持 `Esc` 关闭 | 查完诊断直接开关，无需回退表格 |
| **系统体检 (Doctor)** | 三方一致性深度体检，未受管孤儿条目直接提供**「前往收编 ↗」**导流跳转，100% 健全指标状态卡片 | 消除断链与台账漂移，隐患一键修复 |
| **收编已有 (Adopt)** | 扫描各已装 Agent 既有技能，支持拷贝/移动双模式，100% 健全卡片与一键清空搜索 | 统一收编各处散落的 Skill 到中央仓库 |
| **安装技能 (Add)** | 本地目录/Git 仓库浅克隆，安装前高危注入阻断，安装成功提供下一步直达操作 | 安全入库与一步到位多 Agent 暴露 |
| **技能市场 (Market)** | 内置官方与社区源，**Prompt 深度预览抽屉**（语法高亮与风险分级），**目标 Agent 勾选 Chips 定向安装** | 浏览源内优质技能，定向安装并一键开放 |
| **技能维护 (Update)** | 单技能独立「检查」与「更新」、顶部批量更新、diff 芯片展开**变更文件明细抽屉** | 精细化掌控 Skill 版本演进与文件级改动 |
| **团队协同 (Team)** | 一键填充工作区默认 `./.skillpot.yaml`、预演/导出/对齐，对齐报告统计摘要条 | 全团队 Skill 配置与版本锁毫米级对齐 |
| **策略治理 (Policy)** | 策略总览看板、合规审计、违规项一键修复、规则拓扑网格与内嵌在线 YAML 编辑器 | 组织级合规基线强推与非白名单来源拦截 |

### 现代交互体验规范
- **视口根级锁定与单层平滑滚动**：分离 `matrix-mode`（表格内唯一滚动容器，底栏 Pinned 吸附）与 `panel-mode`（常规面板统一外层单容器平滑滚动），彻底消除浏览器双滚动条与列表多重“套娃”滚动。
- **智能防溢出 Tooltip**：视口边界感知，在底部空间不足时自动向上反转（`.flip-up`），右侧边界水平自动约束，告别截断。
- **实时热响应**：顶栏「⟳ 刷新环境」按钮一键清除缓存并重新探测本机 Agent；「系统体检」Tab 动态红标徽章提示未决问题。
